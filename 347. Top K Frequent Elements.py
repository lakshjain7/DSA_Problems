...
k most frequent. O(n log k) time, O(n + k) space — preferable when k << number
of unique values and you want simple code (or streaming-style processing).
"""
...
def top_k_frequent(nums: List[int], k: int) -> List[int]:
    ...
    for freq in range(len(buckets) - 1, 0, -1):
        for value in buckets[freq]:
            ...
            if len(result) == k:
                return result
    return result  # pragma: no cover (k <= unique elements guarantees early return)
...
if __name__ == "__main__":
    for solve in (top_k_frequent, top_k_frequent_heap):
        # Provided examples (order-insensitive comparison)
        assert sorted(solve([1, 1, 1, 2, 2, 3], 2)) == [1, 2]
        assert solve([1], 1) == [1]
        # Edge cases
        assert sorted(solve([5, 5, 5, 5], 1)) == [5]                  # single unique value
        assert sorted(solve([1, 2, 3, 4], 4)) == [1, 2, 3, 4]         # k == all uniques, ties
        assert sorted(solve([-1, -1, -2, -2, -2, 3], 2)) == [-2, -1]  # negatives
        assert sorted(solve([0, 0, 0, 7, 7, 9], 2)) == [0, 7]         # zero as a value
