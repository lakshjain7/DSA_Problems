...
def levelOrder(root: Optional[TreeNode]) -> List[List[int]]:
    ...
    Returns:
        List of lists, each inner list contains node values at that level.
    """
    if not root:
        return []
    ...
    while queue:
        level_size = len(queue)  # snapshot: number of nodes at this level
        level = []
...
if __name__ == "__main__":
    ...
    # Perfect binary tree: all 3 levels
    root = build_tree([1, 2, 3, 4, 5, 6, 7])
    assert levelOrder(root) == [[1], [2, 3], [4, 5, 6, 7]]
    assert levelOrder_dfs(root) == [[1], [2, 3], [4, 5, 6, 7]]
    ...
    assert levelOrder(r) == [[1], [2], [3]]

    # BFS and DFS produce same result on complex tree
